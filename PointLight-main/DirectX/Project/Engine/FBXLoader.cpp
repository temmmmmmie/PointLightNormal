...
void FBXLoader::LoadMaterial(FbxSurfaceMaterial* _pMtrlSur)
{
	...
	// Diff
	tMtrlInfo.DiffCoeff = GetMtrlData(_pMtrlSur
		, FbxSurfaceMaterial::sDiffuse
		, FbxSurfaceMaterial::sDiffuseFactor);

	// Amb
	tMtrlInfo.AmbCoeff = GetMtrlData(_pMtrlSur
		, FbxSurfaceMaterial::sAmbient
		, FbxSurfaceMaterial::sAmbientFactor);

	// Spec
	tMtrlInfo.SpecCoeff = GetMtrlData(_pMtrlSur
		, FbxSurfaceMaterial::sSpecular
		, FbxSurfaceMaterial::sSpecularFactor);

	// Emisv
	tMtrlInfo.EmvCoeff = GetMtrlData(_pMtrlSur
...
{
	path path_content = PathMgr::GetInst()->GetContentPath();
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			for (size_t k = 0; k < vecPath.size(); ++k)
			{
				...
				path_dest = PathMgr::GetInst()->GetRelativePath(path_dest);
				AssetMgr::GetInst()->Load<ATexture>(path_dest, path_dest);
...
void FBXLoader::CreateMaterial()
{
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			Ptr<AMaterial> pMaterial = AssetMgr::GetInst()->Find<AMaterial>(strPath);
			...
			pMaterial->SetShader(AssetMgr::GetInst()->Find<AGraphicShader>(L"Std3D_DeferredShader"));

			wstring strTexKey = m_vecContainer[i].vecMtrl[j].strDiff;
			Ptr<ATexture> pTex = AssetMgr::GetInst()->Find<ATexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetTexture(TEX_PARAM::TEX_0, pTex);

			strTexKey = m_vecContainer[i].vecMtrl[j].strNormal;
			pTex = AssetMgr::GetInst()->Find<ATexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetTexture(TEX_PARAM::TEX_1, pTex);

			strTexKey = m_vecContainer[i].vecMtrl[j].strSpec;
			pTex = AssetMgr::GetInst()->Find<ATexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetTexture(TEX_PARAM::TEX_2, pTex);

			strTexKey = m_vecContainer[i].vecMtrl[j].strEmis;
			pTex = AssetMgr::GetInst()->Find<ATexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetTexture(TEX_PARAM::TEX_3, pTex);

			pMaterial->SetMaterialCoefficient(
				m_vecContainer[i].vecMtrl[j].DiffCoeff
				, m_vecContainer[i].vecMtrl[j].SpecCoeff
				, m_vecContainer[i].vecMtrl[j].AmbCoeff
				, m_vecContainer[i].vecMtrl[j].EmvCoeff);

			pMaterial->SetDomain(RENDER_DOMAIN::DOMAIN_DEFERRED);

			if (FAILED(pMaterial->Save(PathMgr::GetInst()->GetContentPath() + strPath)))
			{
				continue;
			}			

			AssetMgr::GetInst()->AddAsset(pMaterial->GetKey(), pMaterial.Get());			
		}
	}
}
